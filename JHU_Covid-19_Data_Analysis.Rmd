...
for(i in 1:nrow(total_data_comb)){
 ...
 total_data_comb$active[i] <- sum(global_case_long$value[global_case_long$date_final==total_data_comb$final_date[i]])-total_data_comb$recovered[i]-total_data_comb$deaths[i]
 if(i ==1){
   total_data_comb$uninfected[i] <- 7888000000
 } else {
   total_data_comb$uninfected[i] <- total_data_comb$uninfected[i-1]-total_data_comb$active[i]-total_data_comb$deaths[i]-total_data_comb$recovered[i]
 }
}
