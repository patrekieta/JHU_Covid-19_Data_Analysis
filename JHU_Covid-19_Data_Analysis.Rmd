...
us_data$death_rate <- (us_data$death_sum/us_data$Population)*100000
us_data$death_rate <- ifelse(us_data$death_rate==0 | is.nan(us_data$death_rate) | us_data$death_rate==Inf, yes = 0, no = us_data$death_rate)
...
chart4 <- ggplot(data = us_data_comb)+
  geom_point(aes(x = case_rate, y = death_rate), color = "blue")+
  geom_line(aes(x = case_rate, y = death_case_pred), color = "red")
...
Bias, missing data and changes in reporting standards. Changes in case definition. Inaccuracies in JHU data. too many factors and individual data sources make it hard to accurately model. 
